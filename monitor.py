    ...
    try:
        if message_id:
            ...
            msg = await bot.send_message(
                ...
                parse_mode="Markdown", disable_notification=(not alert)
            )
...
async def process_license(bot: Bot, lic: dict, first_scan: bool = False):
    ...
    if first_scan:
        connecting_msg = await bot.send_message(
            ...
            text=(f"🐷 *Fiscal de Serviço Porco*\n📋 *{label}*\n\n"
                  f"⏳ Conectando ao Robôs365...\nAguarde até 30 segundos."),
            parse_mode="Markdown"
        )

    # Pega cookies do banco
    ...
    if not result["success"]:
        err = result["error"]
        logger.error(f"[{label}] Scrape falhou: {err}")

        # Cookie expirado → avisa com instrução
        if "expirada" in err or "Cookies não" in err:
            await bot.send_message(
                chat_id=chat_id,
                text=(f"⚠️ *Sessão expirada* — *{label}*\n\n"
                      f"Preciso de novos cookies para continuar monitorando.\n\n"
                      f"Use /atualizar\\_cookies para renovar."),
                parse_mode="Markdown"
            )
        else:
            prefix = "❌ *Falha na conexão inicial*\n\n" if first_scan else "🚨 *ERRO NO FISCAL*\n\n"
            await send_or_edit(bot, chat_id, None,
                f"{prefix}📋 *{label}*\n\n`{err}`\n\n🕐 {now}", alert=True)
        return

    robot_status = result["robot_status"]
    bets = result["bets"]
    new_bets = [] if first_scan else [b for b in bets if b["id"] not in seen_bets]
    all_bet_ids = "||".join(b["id"] for b in bets)

    # Mensagem de confirmação no primeiro scan
    if first_scan:
        robot_icon = "🟢" if robot_status == "LIGADO" else "🔴"
        await bot.send_message(
            chat_id=chat_id,
            text=(f"✅ *Fiscal conectado com sucesso!*\n"
                  f"📋 *{label}*\n\n"
                  f"Robô: {robot_icon} {robot_status}\n"
                  f"📊 Apostas encontradas hoje: *{len(bets)}*\n"
                  f"🕐 {now}\n\n"
                  f"A partir de agora vou monitorar essa licença. 🐷"),
            parse_mode="Markdown"
        )
    ...
    new_summary_id = await send_or_edit(
        bot, chat_id,
        None if first_scan else prev_summary_msg_id,
        summary_text, alert=False
    )

    # Primeiro scan: alerta se robô já desligado
    ...
    if not first_scan:
        ...
        if robot_just_off or robot_still_off:
            ...
            new_alert_id = await send_or_edit(bot, chat_id, prev_alert_msg_id, alert_text, alert=robot_just_off)
            update_monitor_state(lid, alert_message_id=new_alert_id)
    ...
    logger.info(f"[{label}] ✓ Robô: {robot_status} | Apostas: {len(bets)}")
